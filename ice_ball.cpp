...
ice_ball::ice_ball(int ball_size, const sf::Vector2f& ball_position, const sf::Vector2f& ball_velocity, int active_time, const sf::Color& ball_color)
    : Ball(ball_size, ball_position, ball_velocity, active_time, ball_color) {}

bool ice_ball::player_hit(const player& p) {
    if (!p.get_alive()) {
        return false;
    }
    ...
    bool has_protection = (defense != nullptr && 
                          dynamic_cast<sword*>(defense) != nullptr);
